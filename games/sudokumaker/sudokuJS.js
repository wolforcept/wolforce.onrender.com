...
(function (window, $, undefined) {
	...
	*/
	$.fn.sudokuJS = function (opts) {
		 ...
		 *-----------*/
		opts = opts || {};
		var extraRules = (opts.extraRules && Array.isArray(opts.extraRules)) ? opts.extraRules : [];
		// normalise: accept either a plain function or {title, extraRuleEval}
		extraRules = extraRules.map(function (r, i) {
			if (typeof r === "function")
				return { title: "extraRule[" + i + "]", extraRuleEval: r };
			if (r && typeof r.extraRuleEval === "function")
				return { title: r.title || ("extraRule[" + i + "]"), extraRuleEval: r.extraRuleEval };
			log("extraRules[" + i + "] is invalid (must be a function or {title, extraRuleEval})");
			return null;
		...
		var initBoard = function (opts) {
			...
			if (!alreadyEnhanced) {
				...
				for (var j = 0; j < boardSize * boardSize; j++) {
					var cellVal = (typeof board[j] === "undefined" || board[j] === 0) ? null : board[j];
		...
		* -----------------------------------------------------------------*/
		...
		/* applyExtraRulesToCandidates
		 * --------------
		 * Applies user-provided extra rules to eliminate candidates.
		 * Each rule is called as: rule(x, y, val, board)
		 *   x   = column index (0-based)
		 *   y   = row index (0-based)
		 *   val = candidate value being tested
		 *   board = the current board array (read-only intent)
		 * If ANY rule returns false for a (x, y, val) combination,
		 * that candidate is removed from the cell.
		 * ALWAYS returns false (like visualEliminationOfCandidates).
		 * -----------------------------------------------------------------*/
		function applyExtraRulesToCandidates() {
			if (extraRules.length === 0) return false;

			for (var cellIndex = 0; cellIndex < boardSize * boardSize; cellIndex++) {
				var boardCell = board[cellIndex];
				// only process empty cells that still have candidates
				if (boardCell.val !== null) continue;

				var x = cellIndex % boardSize;  // column
				var y = Math.floor(cellIndex / boardSize); // row
				var candidatesToRemove = [];

				for (var ci = 0; ci < boardCell.candidates.length; ci++) {
					var candidate = boardCell.candidates[ci];
					if (candidate === null) continue; // already eliminated

					// test candidate against every extra rule
					var allowed = true;
					for (var ri = 0; ri < extraRules.length; ri++) {
						try {
							if (extraRules[ri](x, y, candidate, board) === false) {
								allowed = false;
								break;
							}
						} catch (e) {
							log("extraRules[" + ri + "] threw an error: " + e);
						}
					}
				...
				if (candidatesToRemove.length > 0) {
					removeCandidatesFromCell(cellIndex, candidatesToRemove);
				}
			}
			return false;
		}
		...
		var analyzeBoard = function () {
			...
			while (canContinue) {
				var startStrat = onlyUpdatedCandidates ? (1 + extraRules.length) : 0;
			...
			if (boardError) {
				...
				for (var i = 0; i < usedStrategies.length; i++) {
					...
					if (typeof usedStrategies[i] !== "undefined" && strat) {
		...
		/* attachBoardHelpers
		 * --------------
		 *  Attaches helper methods onto the board array so extra rules can
		 *  query cell values conveniently.
		 *
		 *  board.getVal(x, y) - returns the value of the cell at column x, row y,
		 *    or null if the cell is empty. Returns null for out-of-bounds coordinates
		 *    so callers don't need to bounds-check.
		 * -----------------------------------------------------------------*/
		var attachBoardHelpers = function () {
			board.getVal = function (x, y) {
				if (x < 0 || y < 0 || x >= boardSize || y >= boardSize) return null;
				return board[y * boardSize + x].val;
			};
		};
		...
		// Insert each extraRule as a named strategy at position 1 (after openSingles,
		// before singleCandidate). Each strategy applies only its own rule to eliminate
		// candidates, then checks whether any cell was reduced to a single candidate.
		(function () {
			for (var ri = extraRules.length - 1; ri >= 0; ri--) {
				(function (ruleObj) {
					var stratFn = function () {
						// eliminate candidates forbidden by this rule
						var cellsAffected = [];
						for (var cellIndex = 0; cellIndex < boardSize * boardSize; cellIndex++) {
							var boardCell = board[cellIndex];
							if (boardCell.val !== null) continue;

							var x = cellIndex % boardSize;
							var y = Math.floor(cellIndex / boardSize);
							var toRemove = [];
							...
							for (var ci = 0; ci < boardCell.candidates.length; ci++) {
								...
								try {
									if (ruleObj.extraRuleEval(x, y, candidate, board) === false)
										toRemove.push(candidate);
								} catch (e) {
									log(ruleObj.title + " threw an error: " + e);
								}
							}
							if (toRemove.length > 0) {
								removeCandidatesFromCell(cellIndex, toRemove);
								cellsAffected.push(cellIndex);
								// if this cell now has zero candidates, board is unsolvable
								var remaining = board[cellIndex].candidates.filter(function (c) { return c !== null; });
						...
						if (cellsAffected.length === 0) return false;

						// check whether any cell was reduced to a single candidate
						for (var i = 0; i < board.length; i++) {
							if (board[i].val !== null) continue;
							var possible = [];
							var cands = board[i].candidates;
							for (var j = 0; j < cands.length; j++) {
								if (cands[j] !== null) possible.push(cands[j]);
								if (possible.length > 1) break;
							}
							if (possible.length === 1) {
								setBoardCell(i, possible[0]);
								if (solveMode === SOLVE_MODE_STEP)
									uIBoardHighlightCandidate(i, possible[0]);
								onlyUpdatedCandidates = false;
								return [i];
							}
						}

						// candidates were eliminated but no cell solved yet -
						// return the affected cells so solveFn records this strategy was used
						onlyUpdatedCandidates = true;
						return cellsAffected;
					};
		...
		if (!opts.board) {
			...
			board = (Array.isArray(opts.board[0])) ? [].concat.apply([], opts.board) : opts.board;
			initBoard();
			attachBoardHelpers();
			// validate pre-filled values against extra rules before rendering
			if (extraRules.length > 0) {
				for (var _ci = 0; _ci < boardSize * boardSize; _ci++) {
					var _cell = board[_ci];
					if (_cell.val === null) continue;
					var _x = _ci % boardSize;
					var _y = Math.floor(_ci / boardSize);
					for (var _ri = 0; _ri < extraRules.length; _ri++) {
						try {
							if (extraRules[_ri].extraRuleEval(_x, _y, _cell.val, board) === false) {
								log("extraRules[\"" + extraRules[_ri].title + "\"] violated by pre-filled cell " + _ci + " (row=" + _y + ", col=" + _x + ", val=" + _cell.val + ")");
								if (typeof opts.boardErrorFn === "function")
									opts.boardErrorFn({ msg: "Pre-filled cell at row " + _y + ", col " + _x + " (value " + _cell.val + ") violates extra rule \"" + extraRules[_ri].title + "\"." });
							}
						} catch (e) {
							log(extraRules[_ri].title + " threw an error during pre-fill validation: " + e);
						}
					}
				}
			}
