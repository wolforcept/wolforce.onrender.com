...
// Each entry has the piece Name (matching a Pentomino Name) and the list of
// cells it occupies as [Col, Row] coordinates (0-based, Col=X, Row=Y).
// Example:
// var PrePlaced = [
...
function Initialise() {
    ...
    for (i = 0; i < PrePlaced.length; i++) {
        var ppName = PrePlaced[i].Name;
        var ppCells = PrePlaced[i].Cells;
        // Find the shape index for this name
        var ppShapeIdx = -1;
        for (j = 0; j < Shapes.length; j++) {
            if (Shapes[j].Name === ppName) { ppShapeIdx = j; break; }
        }
        if (ppShapeIdx === -1) {
            Debug("PrePlaced: unknown piece name '" + ppName + "' - skipping");
            continue;
        }
        for (k = 0; k < ppCells.length; k++) {
            var ppCol = ppCells[k][0];
            var ppRow = ppCells[k][1];
            Board.Layout[ppCol][ppRow] = ppShapeIdx;
        }
    }
...
function GetPrePlacedIndices() {
    ...
    for (var i = 0; i < PrePlaced.length; i++) {
        for (var j = 0; j < Shapes.length; j++) {
            if (Shapes[j].Name === PrePlaced[i].Name) {
                indices.push(j);
                break;
            }
        }
